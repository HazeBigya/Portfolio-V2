      ...
      {nodes.map((n, i) => (
        <Fragment key={n.label + i}>
          <motion.div
            ...
            className="relative flex min-w-[120px] flex-col justify-center rounded-xl border border-hairline bg-surface px-4 py-3"
          >
            <span className="absolute left-0 top-3 h-[calc(100%-1.5rem)] w-0.5 rounded-full" style={{ background: accent }} />
            <div className="pl-2 text-sm font-medium text-foreground">{n.label}</div>
            {n.note && <div className="pl-2 text-xs text-muted-foreground">{n.note}</div>}
          </motion.div>
          {i < nodes.length - 1 && (
